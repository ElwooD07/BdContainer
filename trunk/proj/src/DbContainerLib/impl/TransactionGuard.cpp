...
dbc::TransactionGuardImpl::~TransactionGuardImpl()
{
	try
	{
		if (!m_committed)
		{
			TransactionQueryImpl("ROLLBACK " + m_transactionName + ";");
		}
	}
...
{
	TransactionQueryImpl("RELEASE " + m_transactionName + ";");
	m_committed = true;
}
...
void dbc::TransactionGuardImpl::TransactionQueryImpl(const std::string& query)
{
	char* errStr = 0;
